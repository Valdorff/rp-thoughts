...
penalty for a 16 ETH minipool and 0.8 ETH per penalty for an LEB8). This doesn't make sense. MEV
theft doesn't scale with NO deposit size, and there is no reason to allow for strikes for MEV theft.
...
- This value is used to calculate a commission correction that would have resulted in the expected
  ...
  - Assigning MEV to NO with correction factor:
...
- The pDAO votes in a corrected commission to account for MEV; quarterly is likely good enough
...
longer has to protect from MEV theft, it can be much lower, such as 1.5 ETH per minipool (it should
 be in ETH to avoid volatility risk to the protocol, and it needs to at least cover self-slashing to
grief). If the total value of (staked ETH minus amount owed) drops below the threshold, RPL should
be auctioned to bring them back above that threshold. If there is no longer any RPL to auction, a
minipool should be force exited.
...
potential for MEV theft again, which means it needs penalties. This is still possible by using the
Node Level Collateral previuosly mentioned, but it would not be possible for small stakers.

It's important to point out that this solution doesn't depend on the oDAO at all! There is no need
for penalizing, tracking MEV, or manual force exiting (this could be done with a smart contract and
a keeper bot that anyone can run).
