...
def intended_spend(curr_pie_d, prop_pie_d):
    ...
    # TODO: pie chart of categories
    # TODO: pie chart with all spend including categories
    fig, ax = plt.subplots(1)
    xx = [0.5 * (int(k.split('-')[0]) + int(k.split('-')[1])) for k in curr_pie_d.keys()]
    bins = [int(k.split('-')[0]) for k in curr_pie_d.keys()] + [150]
    _n, _bins, patches = ax.hist(xx, bins=bins, weights=curr_pie_d.values(), rwidth=0.8)
    colors = ['LightSkyBlue'] + ['Blue'] * 3 + ['Navy'] * (len(xx) - 4)
    ...
    _n, _bins, patches = ax.hist(xx, bins=bins, weights=prop_pie_d.values(), rwidth=0.8)
    colors = ['LightSkyBlue'] + ['Blue'] * 3 + ['Navy'] * (len(xx) - 4)
    for i, patch in enumerate(patches):
        patch.set_fc(colors[i])
        patch.set_ec(colors[i])
    ...
    for index, row in df.iterrows():
        ...
        for lowend in range(10, 146, 5):
            ...
            if (row['peth_pct'] < ((lowend + 5) / 100)) or (lowend == 145):
                ...
                break
            # TODO figure out pie up to this sections endpoint; subtract pie so far
            temprow = {
